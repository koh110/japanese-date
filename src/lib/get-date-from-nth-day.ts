...
// n: 第n
// day: 0(日) -> 6(土)
export const getDateFromNthDay = (year: number, month: number, n: number, day: number) => {
  const firstDateInstance = dayjs().year(year).month(month).date(1).hour(0).minute(0).second(0);
